...
from sympy import print_latex, Rational, Symbol, Dummy
...
def get_free_indicies(tex_string):
    ...
    for index, count in zip(unique_indices, counts):
        if count == 1:
            ...
            free[above_fermi * "upper" + (1 - above_fermi) * "lower"].append(
                symbols(index, above_fermi=above_fermi)
            )
...
def fix_sympy_expr(expr, fix_sympy_expr):
    ...
    for index in fix_sympy_expr.values():
        if index != []:
            permutation_operators.append(PermutationOperator(index[0], index[1]))
