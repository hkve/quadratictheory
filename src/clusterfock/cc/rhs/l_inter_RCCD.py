...
def lambda_amplitudes_intermediates_rccd(t2, l2, u, f, v, o):
    ...
    r2 += 2 * np.einsum(
        "ablk,lkij->abij", l2, tau0, optimize=True
    )
    ...
    r2 += 2 * np.einsum(
        "ijlk,lkab->abij", tau1, u[o, o, v, v], optimize=True
    )
    ...
    tau23 += np.einsum(
        "acki,kjcb->ijab", l2, tau3, optimize=True
    )
    ...
    tau6 += np.einsum(
        "ijbc,ijca->ab", tau5, u[o, o, v, v], optimize=True
    )
    ...
    tau8 += np.einsum(
        "cb,caij->ijab", tau7, l2, optimize=True
    )
    ...
    tau10 += np.einsum(
        "kjab,kiab->ij", tau9, u[o, o, v, v], optimize=True
    )

    tau11 = zeros((N, N))

    tau11 += np.einsum(
        "ji->ij", tau10, optimize=True
    )
    ...
    tau15 += np.einsum(
        "kjcb,kica->ijab", tau9, u[o, o, v, v], optimize=True
    )
    ...
    tau12 += np.einsum(
        "kj,abki->ijab", tau11, l2, optimize=True
    )
    ...
    r2 += 4 * np.einsum(
        "ijab->abij", tau17, optimize=True
    )

    r2 -= 2 * np.einsum(
        "ijba->abij", tau17, optimize=True
    )

    r2 -= 2 * np.einsum(
        "jiab->abij", tau17, optimize=True
    )

    r2 += 4 * np.einsum(
        "jiba->abij", tau17, optimize=True
    )
    ...
    tau19 += np.einsum(
        "ac,jicb->ijab", tau18, u[o, o, v, v], optimize=True
    )
    ...
    tau21 += np.einsum(
        "ik,jkab->ijab", tau20, u[o, o, v, v], optimize=True
    )
    ...
    r2 += 2 * np.einsum(
        "ijab->abij", tau22, optimize=True
    )

    r2 -= 4 * np.einsum(
        "ijba->abij", tau22, optimize=True
    )

    r2 -= 4 * np.einsum(
        "jiab->abij", tau22, optimize=True
    )

    r2 += 2 * np.einsum(
        "jiba->abij", tau22, optimize=True
    )
    ...
    r2 += 2 * np.einsum(
        "balk,jilk->abij", l2, u[o, o, o, o], optimize=True
    )
