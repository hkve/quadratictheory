...
def amplitudes_intermediates_ccd(t2, u, f, v, o):
    ...
    tau4 += np.einsum(
        "jk,abik->ijab", tau3, t2, optimize=True
    )
    ...
    tau6 += np.einsum(
        "bc,caji->ijab", tau5, t2, optimize=True
    )
    ...
    tau13 += np.einsum(
        "caik,jkbc->ijab", t2, tau7, optimize=True
    )
    ...
    tau9 += np.einsum(
        "caki,jkbc->ijab", t2, tau8, optimize=True
    )
    ...
    r2 -= np.einsum(
        "ijab->abij", tau10, optimize=True
    ) / 4

    r2 += np.einsum(
        "ijba->abij", tau10, optimize=True
    ) / 4

    r2 += np.einsum(
        "jiab->abij", tau10, optimize=True
    ) / 4

    r2 -= np.einsum(
        "jiba->abij", tau10, optimize=True
    ) / 4
    ...
    tau12 += np.einsum(
        "acik,jkbc->ijab", t2, tau11, optimize=True
    )
    ...
    r2 += np.einsum(
        "bakl,klji->abij", t2, tau15, optimize=True
    ) / 4

    tau15 = None

    r2 -= np.einsum(
        "cdji,badc->abij", t2, u[v, v, v, v], optimize=True
    ) / 2
